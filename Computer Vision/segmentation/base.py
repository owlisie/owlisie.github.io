...
def evaluate_and_visualize_samples(model, dataset, device, save_dir="comparison_results"):
...
if __name__ == "__main__":
    ...
    dataset = SputumSegmentationDataset(image_dir, label_dir, image_size=(256, 256))

    test_subset = Subset(SputumSegmentationDataset(test_image_dir, test_label_dir, image_size), range(25))
    test_loader = DataLoader(test_subset, batch_size=1)
    model = UNet()

    train(model, dataset, device, epochs=100, save_path="./weight/unet_base.pth")

    # 불러오기
    model.load_state_dict(torch.load("./weight/unet_base.pth", map_location=device))

    test_dataset = SputumSegmentationDataset(test_image_dir, test_label_dir, image_size)
    model.load_state_dict(torch.load("weight/unet_base.pth", map_location=device))
